...
git clone https://github.com/cvetomirgrigorov/servicenow-mcp.git && cd servicenow-mcp
...
```typescript
// lib/servicenow.ts
const SNMCP_URL = import.meta.env.VITE_SNMCP_URL || 'http://localhost:3100';
const SNMCP_KEY = import.meta.env.VITE_SNMCP_KEY || '';

export async function callTool<T>(tool: string, params: Record<string, unknown> = {}): Promise<T> {
  const res = await fetch(`${SNMCP_URL}/api/tool`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(SNMCP_KEY ? { Authorization: `Bearer ${SNMCP_KEY}` } : {}),
    },
...
const SNMCP_URL = Deno.env.get('SNMCP_URL') || '';
const SNMCP_KEY = Deno.env.get('SNMCP_API_KEY') || '';
...
serve(async (req: Request) => {
  ...
  const upstream = await fetch(`${SNMCP_URL}/api/tool`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${SNMCP_KEY}`,
    },
...
Deploy to Supabase Edge Functions and set the `SNMCP_URL` and `SNMCP_API_KEY` secrets.
...
| `SNMCP_API_KEY` | Bearer token required on all API requests | (none — no auth) |
