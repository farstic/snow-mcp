...
// ─── Security helpers ─── (extracted to ./security.ts for unit testing)
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('tools:execute', async (_event, toolName: string, args: Record<string, unknown>) => {
    const start = Date.now();
    const result = await serverManager.routeToolInvocation(toolName, args);
    configStore.appendAuditLog({
      ts: new Date().toISOString(),
      event: 'tool:execute',
      tool: toolName,
  ...
  // The renderer handles the tool execution loop (tool_use → routeToolInvocation → tool_result → repeat).
  ipcMain.handle('chat:send', async (_event, params: {
    ...
    const systemPrompt = toolDefs && toolDefs.length > 0
      ? `You are ServiceNow MCP Toolkit, an AI assistant for ServiceNow. You have access to tools that query and modify a ServiceNow instance.
...
HOW TO QUERY DATA:
- Use "snow_core_records_query" tool with the correct table name:
