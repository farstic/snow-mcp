...
export class ServerManager {
   ...
   * In development: ../dist/server.js (parent servicenow-mcp project)
  ...
  async start(instance: InstanceConfig): Promise<{ success: boolean; error?: string }> {
    ...
    if (!existsSync(serverPath)) {
      return { success: false, error: `Server not found at ${serverPath}. Run "npm run build" in the servicenow-mcp root directory.` };
    }
   ...
   */
  async routeToolInvocation(name: string, args: Record<string, unknown>): Promise<{ success: boolean; result?: unknown; error?: string }> {
