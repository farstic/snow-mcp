...
Step-by-step setup for connecting ServiceNow MCP Toolkit to each supported AI client.
...
**Verify**: Open Claude Desktop → Settings → Developer → MCP Servers → `servicenow-mcp` should show green.
...
**Verify**: Open Cursor → Settings → MCP → `servicenow-mcp` should appear in the list.
