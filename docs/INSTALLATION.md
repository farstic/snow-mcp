...
| Platform | Download |
|----------|----------|
| **macOS** | [servicenow-mcp.dmg](https://github.com/cvetomirgrigorov/servicenow-mcp/releases/latest) |
| **Windows** | [servicenow-mcp-Setup.exe](https://github.com/cvetomirgrigorov/servicenow-mcp/releases/latest) |
| **Linux** | [servicenow-mcp.AppImage](https://github.com/cvetomirgrigorov/servicenow-mcp/releases/latest) · [servicenow-mcp.deb](https://github.com/cvetomirgrigorov/servicenow-mcp/releases/latest) |
...
```bash
git clone https://github.com/cvetomirgrigorov/servicenow-mcp.git
cd servicenow-mcp
npm install
npm run build
```
...
Search order: `SNMCP_ORG_CONFIG` env var → `/etc/servicenow-mcp/org.json` → `./servicenow-mcp.org.json`

---

## HTTP API Server

Run the server with `TRANSPORT=http` to expose all tools as a REST API for web apps:

```bash
TRANSPORT=http PORT=3100 npm start

# With API key auth:
SNMCP_API_KEY=my-secret-key TRANSPORT=http PORT=3100 npm start
```
...
2. Ask: *"Get the schema for the incident table"* — should call `snow_core_table_schema_read`
3. Ask: *"Show me 5 open P1 incidents"* — should call `snow_core_records_query`
...
- Issues: https://github.com/cvetomirgrigorov/servicenow-mcp/issues
...
| `SNMCP_API_KEY` | (none) | Bearer token for API auth |
...
| Variable | Description |
|----------|-------------|
| `SNMCP_ORG_CONFIG` | Path to `servicenow-mcp.org.json` |

---

**Need help?** Check [Troubleshooting](#troubleshooting) above or file an issue at https://github.com/cvetomirgrigorov/servicenow-mcp/issues
