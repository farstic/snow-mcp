      ...
      },
      {
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Author each subflow as its own FlowSpec (no trigger; typed `inputs` / `outputs`) and build it before the main flow, so the main flow can call it with a `subflow` step.',
            '',
            '### Step 5 — Author the FlowSpec with Error Handling',
            'Use **snow_flow_catalog_read** to look up the exact trigger, action and flow-logic names and their input types, then author a FlowSpec v1 (symbolic pills only: `{{trigger.current.<field>}}`, `{{steps.<key>.<Output>}}`, `{{loop.<key>.item}}`, `{{vars.<name>}}`) with:',
            ...
            '',
            '### Step 6 — Plan, Build and Verify the Flow',
            'Run **snow_flow_plan** on the spec (dry run — never writes) and review the decoded plan, the pill table and the update-set capture steps with the user. Only after an explicit write approval, run **snow_flow_build** (explicit `instance`, an in-progress non-default `update_set`, `activate:false` unless activation is separately approved), then **snow_flow_verify** to read the flow back. The flow is written as a draft; activate it in Workflow Studio. Report:',
